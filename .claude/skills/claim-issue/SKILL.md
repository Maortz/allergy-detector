...
description: >
  ...
  issue in Maortz/allergy-detector before starting implementation, to prevent
...
```bash
gh label list --repo Maortz/allergy-detector | grep agent-in-progress \
  || gh label create agent-in-progress \
       --repo Maortz/allergy-detector \
...
```bash
gh pr list --repo Maortz/allergy-detector --state open --search "issue-<N>"
```
...
```bash
gh issue view <N> --repo Maortz/allergy-detector --json labels
```
