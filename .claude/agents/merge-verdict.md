...
name: merge-verdict
description: >
  Audits the review gate for every open PR in Maortz/allergy-detector and re-triggers CI (via empty commit) for PRs that pass review but have no current green run. Never merges or approves. Triggers on /merge-verdict, "which PRs are ready to merge", or from the agent-pipeline.
