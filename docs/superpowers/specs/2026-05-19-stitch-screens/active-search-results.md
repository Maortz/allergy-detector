...
<!-- PENDING DECISION: bottom-nav tab composition on active-search-results screen -->
...
This differs from the canonical 4-tab set in `_components-glossary.md#bottom-nav` which lists: בית / סריקה / קהילה / מועדפים. See `## INCONSISTENCY FOUND` below.
...
| Status pill — Safe | <!-- PENDING DECISION: status-pill label variant on active-search-results --> Background `#DCFCE7`, text `#15803D` | Inter SemiBold 12 pt | `check_circle` 16 pt | "בטוח לצריכה" | Differs from glossary "בטוח" — see INCONSISTENCY |
| Status pill — Avoid detail | <!-- PENDING DECISION: status-pill label variant on active-search-results --> Background `#FEE2E2`, text `#991B1B` | Inter SemiBold 12 pt | `warning` 16 pt | "מכיל אגוזי לוז" | Shows allergen name, not verdict label |
| Status pill — Caution detail | <!-- PENDING DECISION: status-pill label variant on active-search-results --> Background `#FEF9C3`, text `#A16207` | Inter SemiBold 12 pt | `info` 16 pt | "חשש לגלוטן" | Shows allergen concern, not verdict label |
| Product thumbnail | `BorderRadius.circular(8)` | — | — | — | 56×56 pt, `BoxFit.cover`; RTL trailing (left) |
| Bottom nav | see _components-glossary.md#bottom-nav | Inter 11 pt | varies | "חיפוש" active | Tab composition differs — see INCONSISTENCY |
...
### 7.1 StatusPill label: verdict vs. allergen detail
**Question:** The glossary defines `status-pill` labels as "בטוח" / "זהירות" / "להימנע". This screen shows "בטוח לצריכה", "מכיל אגוזי לוז", "חשש לגלוטן" — longer, context-specific strings. Are these a distinct `StatusDetailPill` widget, or should `StatusPill` accept an optional `detailLabel` override?
**Impact:** affects the shared `StatusPill` widget interface. Pending decision.

### 7.2 Bottom-nav tab 4 identity (Settings vs. Favorites)
This screen shows "הגדרות" (Settings) as the 4th (leftmost) tab, contradicting DD-2 which resolves tab 4 = "מועדפים" (Favorites). Additionally, the tab set here is: חיפוש / סריקה / מועדפים / הגדרות — **5 distinct concepts across 4 slots**, and "חיפוש" replaces "בית" as the active tab. See `## INCONSISTENCY FOUND` section below.

### 7.3 "חיפוש" vs. "סריקה" tabs
The glossary bottom-nav shows "סריקה" at index 1 as the scan/search tab. This screen has **two separate tabs**: "חיפוש" (Search, active) and "סריקה" (Scan). This implies a 5-tab nav, or that "חיפוש" replaces "בית" when in search context, or that the tab set is context-sensitive. Needs resolution.
...
### INC-1 · Status pill label text

- **Element:** `status-pill` label content.
- **This screen (active-search-results):** pill label = context-specific Hebrew string — "בטוח לצריכה" (Safe), "מכיל אגוזי לוז" (Avoid with allergen name), "חשש לגלוטן" (Caution with allergen concern). Pill structure is otherwise identical to glossary.
- **Glossary variant (`_components-glossary.md#status-pill`):** label = verdict keyword only — "בטוח" / "זהירות" / "להימנע".
- **Screen ID:** `45d081ae18b143ca8e15b12469468d9a`
- **Resolution needed:** determine whether this is a subtype (`StatusDetailPill`) or a `StatusPill` prop extension.

### INC-2 · Bottom-nav tab set and active tab identity

- **Element:** bottom navigation bar tab composition.
- **This screen (active-search-results):** 4 tabs observed RTL right→left: "חיפוש" (Search, active, index 0), "סריקה" (Scan, index 1), "מועדפים" (Favorites, index 2), "הגדרות" (Settings, index 3 leftmost). Tab 1 is "חיפוש" not "בית".
- **Glossary / DD-2 canonical form (`_components-glossary.md#bottom-nav`, `_design-decisions.md#dd-2`):** 4 tabs: "בית" / "סריקה" / "קהילה" / "מועדפים". Settings is NOT a bottom-nav tab. No "חיפוש" tab.
- **Screen ID:** `45d081ae18b143ca8e15b12469468d9a`
- **Resolution needed:** determine whether (a) this screen replaces "בית" with "חיפוש" contextually, (b) there is a 5-tab nav on some screens, or (c) this is a Stitch design artefact of an older nav layout that predates DD-2.
