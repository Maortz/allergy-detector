...
| 17 | Contact Us (Updated) | `contact-us` | `contact_screen.dart` | ✓ | ✓ | ✓ | ⚠ (CC1–CC7, §7.7 — partial; CC4 false-success fixed #17; subject picker added §4.3 #84; prefix-icon inset `EdgeInsetsDirectional` #90; CC6 email validation regex-based via `Validators.isValidEmail` + spec §5.3 error copy, #91) | ⚠ (V-Art 2026-06-14, #218: the "שלחו לנו הודעה" form (name/email/subject-picker/message/send) matches the art, but the art's "פרטי התקשרות" direct-contact block is absent — no hero intro card (CC1), no email/phone/hours/social ContactMethodRows with mailto:/tel: (CC2); §7.7. App-bar brand "בדיקת אלרגנים" is a stale DD-8 artifact) | `5a9bc40c2d8a46c7b760d2725cde2cf4` |
...
| 21 | Admin Navigation Drawer (Right) | `nav-drawer-admin` | `admin_navigation_drawer.dart` | ✓ | ✓ | ✓ | ✓ (DA1–DA12 built #21, gated on isAdmin; version via PackageInfo §7.2, לוח בקרה active-default §5.4; #26 wired all Tier-3 admin rows to their pushed destination screens (placeholder bodies), so the coming-soon snackbar is gone; #120 made the root drawer's `activeDestination` reflect the live admin screen (§5.4) — `MainContainer` now tracks `_activeAdminDestination`, sets it when a Tier-3 row is pushed and resets to dashboard on pop, replacing the hardcoded `dashboard`) | ⬜ | `b4224114bb2e4ff6a2cca1db65a401f6` |
