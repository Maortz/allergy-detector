...
| 15 | Settings & Profile | `settings-profile` | `settings_screen.dart` | ✓ | ✓ | ✓ | ⚠ (§7.8 — filter ST4–ST7 wired+persisted #16, `avoidOnly`→`showAll` rename #70; filter→list display wired home+search #41; #168 added an Appearance picker (בהיר/כהה/מערכת) backed by `ThemeService` (SharedPreferences `appearance_mode`) — `MyApp` now ships `darkTheme: buildDarkAppTheme()` + `themeMode` wired from the picker through `AppShell`→`MainContainer`→`SettingsScreen`; remaining ST1–ST3,ST8–ST12 pending; #177 wrapped the screen body in a Scaffold + RTL AppBar ("פרופיל") — it is pushed via a bare MaterialPageRoute from the user drawer and previously had no Scaffold/Material ancestor, crashing on entry on Android; widget tests masked it by supplying their own Scaffold, now guarded by a bare-MaterialApp regression test) | ⚠ (V-Art 2026-06-14, #184: diverges from the Stitch art — avatar 96 pt + edit-pencil vs art's 64 pt + verified badge + "ערוך פרופיל" link (ST1/ST3), filter section header/copy + chips lack the status-pill palette (ST4/ST5/ST7), extra "נהל מותגים" admin row not in art (ST8), only "נהל אלרגיות" tinted vs per-row tints (ST9), chevron_right vs art's chevron_left (ST10), logout is a faint OutlinedButton vs the art's filled light-red button (ST11); §7.8) | `3a2bc2f1aac1444886d44def38b72bc4` |
