...
| 11 | Community Hub | `community-hub` | `community_screen.dart` | ✓ | ✓ | ✓ | ⚠ (CH1–CH13, §7.8; #137 fixed `didUpdateWidget` to re-sync `_localQueue` on *every* `pendingReviews` identity change incl. a null reset — a logout/data-clear now empties the queue instead of leaving phantom review items; guarded by a new `community_screen_test` case) | ⬜ | `a8c9931205604870a6ecee4456c6e808` |
| 12 | Community Review | `community-review` | `community_review_screen.dart` | ✓ | ✓ | ✓ | ⚠ (CR1–CR11, §7.6 — built #39; #46 dropped the full-body spinner for per-button loading: the tapped approve/reject button shows an inline 20×20 spinner and both decision buttons + reason field disable while a submit is in-flight, killing the double-submit race) | ⬜ | `521b195cd91443849b0f983487ef5f9c` |
